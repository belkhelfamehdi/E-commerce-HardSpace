            ...
            <div class="py-4 text-gray-500 dark:text-gray-400">
                <ul class="mt-6">
                    <li class="relative px-6 py-3">
                        @if (Route::is('supplier.dashboard'))
                        <span class="absolute inset-y-0 left-0 w-1 bg-pcolor rounded-tr-lg rounded-br-lg"
                            aria-hidden="true"></span>
                            @endif
                        <a href="{{route('supplier.dashboard')}}" class="inline-flex items-center w-full text-sm font-semibold @if (Route::is('supplier.dashboard')) text-gray-800 @endif transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 dark:text-gray-100"
                            href="">
                    ...
                    <li class="relative px-6 py-3">
                        @if (Route::is('supplier.products'))
                        <span class="absolute inset-y-0 left-0 w-1 bg-pcolor rounded-tr-lg rounded-br-lg"
                            aria-hidden="true"></span>
                            @endif
                            @if (Route::is('supplier.products.create'))
                        <span class="absolute inset-y-0 left-0 w-1 bg-pcolor rounded-tr-lg rounded-br-lg"
                            aria-hidden="true"></span>
                            @endif
                            @if (Route::is('supplier.products.edit'))
                        <span class="absolute inset-y-0 left-0 w-1 bg-pcolor rounded-tr-lg rounded-br-lg"
                            aria-hidden="true"></span>
                            @endif
                        <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 @if (Route::is('supplier.products')) text-gray-800 @endif @if (Route::is('supplier.products.create')) text-gray-800 @endif @if (Route::is('supplier.products.edit')) text-gray-800 @endif hover:text-gray-800 dark:hover:text-gray-200"
                            href="{{route('supplier.products')}}">
                            <svg class="w-5 h-5" aria-hidden="true" fill="none" stroke-linecap="round"
                                stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                                <path
                    ...
                    <div class="grid gap-6 mb-8 md:grid-cols-2 xl:grid-cols-4">
                        ...
                        <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
                            ...
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                                    {{ \App\Models\Product::where('supplier_id', auth()->user()->id)->count() }}
                                </p>
                            </div>
                        </div>
                    </div>
